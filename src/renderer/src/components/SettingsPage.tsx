...
export function SettingsPage(): JSX.Element | null {
  ...
  // One-line guide health for the Settings row that links to the Guide & EPG surface — enough to
  // answer "is my guide working?" without opening it; the detail (per-source match counts, manual
  // maps) lives on that page.
  const guideStatusLine = `${settings.customEpgUrls.length} custom source${
    settings.customEpgUrls.length === 1 ? '' : 's'
  } · ${
    providerGuideAvailable === true
      ? 'provider guide available'
      : providerGuideAvailable === false
        ? 'provider guide unavailable'
        : 'provider guide not checked'
  }`
      ...
      <div className="modal-card settings-card" onClick={(e) => e.stopPropagation()}>
        ...
        <section className="settings-section">
          ...
          <p className="settings-hint">
            ...
            ({guideStatusLine}). Open it whenever a channel is missing listings, or to add another
            XMLTV guide.
          </p>
