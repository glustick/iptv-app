...
- **Match one channel in one place**: right-click any channel → **EPG match…** shows what it is mapped to now and a shortlist of **scored** guide-channel candidates — including close ones below the automatic threshold, each labelled with its percentage — with the source's whole guide one click away behind a search. The Guide & EPG page keeps the overview instead: one row per source showing its guide size, how much it matched and how many manual mappings it holds, with a **Hide guide / Show guide** switch that takes a source out of the pool without deleting it
...
- **Channel health at a glance**: a channel whose feed isn't actually live — a provider serving a finished clip on repeat on a live URL, which is a real and common failure mode — is flagged on its row, explained in the preview, and can be filtered out with one click. Detected from the playlist itself (an `#EXT-X-ENDLIST` live channel cannot be live), probed lazily as you browse, and never asserted from a failed request
- **Right-click a channel** for Hide/Show and **EPG match…**, which jumps straight into the guide mapping editor with that channel already picked — instead of hunting for it again in a 30,000-channel list. Hidden channels are managed from the same menu, so no per-row clutter
